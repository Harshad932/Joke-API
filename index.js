...
app.get("/jokes/:id",(req,res)=>{
  ...
  var j=jokes.find((joke)=>joke.id===s);
...
app.get("/filter",(req,res)=>{
  ...
  var s=req.query.type;
  var t=s.substring(0,1).toUpperCase()+s.substring(1,s.length); 
  console.log(t);
  var j=jokes.filter((joke)=>joke.jokeType===t);
  res.json(j);
...
app.post("/jokes",(req,res)=>{
  ...
  jokes.push({id:id,jokeText:j,jokeType:t});
...
app.put("/jokes/:id",(req,res)=>{
  ...
  else
  {
    for (var i = 0; i < jokes.length; i++) {
      if (jokes[i].id === id) {
        index=i;
        delete jokes[i].jokeText;
        delete jokes[i].jokeType;
        break;
      }
    }
  }
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  var i=jokes.findIndex((joke)=>joke.id==id);
